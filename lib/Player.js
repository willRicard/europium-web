class Player {
    ...
    sendCorrect(correct, actualAnswer) {
		...
		return new Promise((resolve) => {
			socket = this.socket;
			socket.broadcast.emit('answer', {name: this.name, correct: correct, actual: actualAnswer});
			socket.emit('answer', {name: this.name, correct: correct, actual: actualAnswer});
